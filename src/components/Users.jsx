...
import { addUser, filterUsers , deleteUser , deleteAll } from '../store/UsersSlice'
function Users() {
  ...
  return (
    <>
      <form>
        ...
        <button type='button' onClick={()=> dispatch(deleteAll())} >Delet All</button>
      </form>
      <hr></hr>
...

          {
            filtredUsers.length === 0 ? users.map((ele) => {
              return (<tr>
                ...
                <td>{ele.name}</td>
                <td>{ele.location}</td>
                <button onClick={()=> dispatch(deleteUser(ele.code))}>Delete</button>
              </tr>)
            })
